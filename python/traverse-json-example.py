...
for item in participants:
    participant_trials = []
    participant_trials = get_clinical_trials(item['age'], item['gender'], item['zipcode'], distance)
    # participant_trials.append(results)
    
    gender = participant_trials[0]['eligibility']['structured'].get('gender', '')

    output = participant_trials[0]['eligibility']['unstructured']
    description_list = []
    for item in output:
        description_list.append(item.get("description", ""))
        

    # desc_big_str = '~'.join(description_list)

    print (description_list)
    # print(json.dumps(description_list, indent=2))

# outfile = "../datasets/participant_trials_test.json" 
# file = open(outfile, "w")
